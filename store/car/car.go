...
func (s CarStore) CreateCar(ctx context.Context, carReq *models.CarRequest) (models.Car, error) {
	...
	err := s.db.QueryRowContext(ctx, "SELCT id FROM engine WHERE id = $1", carReq.Engine.EngineID).Scan(&engineID)
	if err != nil {
	...
	//performing atomicity property
	//so that if there is some problem in inserting values in any of the car or engine table
	//then the whole process will be rollbacked and nothing will be inserted
	//we use transactions for atomicity property
...
func (s CarStore) UpdateCar(ctx context.Context, id string, carReq *models.CarRequest) (models.Car, error) {
	...
	query := `
		UPDATED car SET name = $2, year = $3, fuel_type = $5, engine_id = $6, price = $7, updated_at = $8
		WHERE id = $1
		RETURN id, name, year, brand, fuel_type, engine_id, price, created_a, updated_at
	`
	err = tx.QueryRowContext(ctx, query,
		id,
		carReq.Name,
